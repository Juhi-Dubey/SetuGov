...
class AIService:
    ...
    async def analyze_challenge(
        ...
        response = self._parse_challenge_response(raw, request=request)
    ...
    async def explain_match(
        ...
        try:
            return MatchExplanationResponse(
                score=score,  # deterministic — NOT from LLM (LLM score ignored)
                why_matched=_sanitize_claim(str(raw.get("why_matched") or "No explanation provided.")),
                strengths=[_sanitize_claim(s) for s in _extract_str_list(raw.get("strengths"))],
                concerns=[_sanitize_claim(c) for c in _extract_str_list(raw.get("concerns"))],
                missing_information=[_sanitize_claim(m) for m in _extract_str_list(raw.get("missing_information"))],
                deployment_considerations=[_sanitize_claim(d) for d in _extract_str_list(raw.get("deployment_considerations"))],
            )
    ...
    async def analyze_proposal(
        ...
        # 1. Authoritative Cost & Timeline Preservation
        # If user provided cost/timeline in request.proposal, strictly preserve it.
        # If not provided by user, do not accept invented LLM values (remain None).
        ...
        if request.proposal and request.proposal.implementation_timeline and request.proposal.implementation_timeline.strip():
            final_timeline = request.proposal.implementation_timeline.strip()
        else:
            final_timeline = None

        # 2. Extract and sanitize qualitative text fields
        exec_summary = _sanitize_claim(str(raw.get("executive_summary") or "No summary provided.")).strip()
        tech_approach = _sanitize_claim(str(raw.get("technical_approach"))).strip() if raw.get("technical_approach") else None
        expected_impact = _sanitize_claim(str(raw.get("expected_impact"))).strip() if raw.get("expected_impact") else None
        tech_readiness = _sanitize_technology_readiness(raw.get("technology_readiness"), request)

        # 3. Parse and sanitize risks
        risks = _parse_proposal_risks(raw.get("risks"))

        # 4. Extract, sanitize, and reconcile list fields
        ...
        questions = [_sanitize_claim(q) for q in _extract_str_list(raw.get("questions_for_evaluator"))]
    ...
    async def interpret_pilot(
        ...
        # 3. Extract and sanitize qualitative fields using resilient helpers
        overall_assessment = _sanitize_claim(
            str(raw.get("overall_assessment") or "No qualitative assessment provided.")
        ).strip()
        observations = [_sanitize_claim(o) for o in _extract_str_list(raw.get("observations"))]
        concerns = [_sanitize_claim(c) for c in _extract_str_list(raw.get("concerns"))]
        ...
        recommended_actions = [_sanitize_claim(a) for a in _extract_str_list(raw.get("recommended_actions"))]

        # 4. Build response — calculations from Python, interpretation from LLM
        try:
