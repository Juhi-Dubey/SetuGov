      ...
      <div className="space-y-5 sm:space-y-6">
        ...
        {state === "success" && data && (
          <>
            ...
            {(analytics?.financials || analytics?.budget) && (() => {
              const fin = analytics.financials || {
                total_paid_budget: analytics.budget?.paid_amount || 0,
                total_allocated_budget: analytics.budget?.allocated_budget || 0,
                budget_utilization_rate: analytics.budget?.utilization_percentage || 0,
                total_pending_budget: analytics.budget?.pending_amount || 0,
              };
              const ms = analytics.milestone_analytics || {
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">
                          ₹{Number(fin.total_paid_budget || 0).toLocaleString("en-IN")}
                          <span className="text-sm font-normal text-slate-400"> / ₹{Number(fin.total_allocated_budget || 0).toLocaleString("en-IN")}</span>
                        </p>
                      </div>
                      <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-bold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                        {fin.budget_utilization_rate}% Disbursed
                      </span>
                    </div>
                    <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                      <div
                        className="h-full bg-emerald-500 transition-all duration-500"
                        style={{ width: `${Math.min(100, Number(fin.budget_utilization_rate || 0))}%` }}
                      />
                    </div>
                    <div className="mt-2.5 flex justify-between text-xs text-slate-400">
                      <span>Disbursed: ₹{Number(fin.total_paid_budget || 0).toLocaleString("en-IN")}</span>
                      <span>Pending: ₹{Number(fin.total_pending_budget || 0).toLocaleString("en-IN")}</span>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-4.5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">
                          {ms.completed_milestones || 0}
                          <span className="text-sm font-normal text-slate-400"> / {ms.total_milestones || 0} Delivered</span>
                        </p>
                      </div>
                      <span className="rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-bold text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300">
                        {ms.completion_rate || 0}% Complete
                      </span>
                    </div>
                    <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                      <div
                        className="h-full bg-indigo-600 transition-all duration-500"
                        style={{ width: `${Math.min(100, Number(ms.completion_rate || 0))}%` }}
                      />
                    </div>
                    <div className="mt-2.5 flex justify-between text-xs text-slate-400">
                      <span>In Progress: {ms.in_progress_milestones || 0}</span>
                      <span>Pending: {ms.pending_milestones || 0}</span>
                    </div>
                  </div>
                </section>
              );
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="inline-flex w-fit items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
          Total {total} tracked pilot project{total === 1 ? "" : "s"}
        </span>
      </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <HealthCard
          ...
          bgTint="bg-emerald-50/40 border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900/30"
          icon={CheckCircle2}
        />
        ...
        <HealthCard
          ...
          bgTint="bg-amber-50/40 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/30"
          icon={AlertTriangle}
        />
        ...
        <HealthCard
          ...
          bgTint="bg-rose-50/40 border-rose-100 dark:bg-rose-950/20 dark:border-rose-900/30"
          icon={AlertOctagon}
        />
      </div>
    </motion.section>
...
function HealthCard({ label, count, total, color, icon: Icon, badgeColor, bgTint }) {
  const percentage = total > 0 ? Math.round((count / total) * 100) : 0;

  return (
    <div className={`rounded-xl border p-3.5 transition-all ${bgTint}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 min-w-0">
          <Icon className={`h-4 w-4 shrink-0 ${badgeColor}`} aria-hidden="true" />
...
function EmptyChallenges({ onCreateChallenge }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
      ...
      <h3 className="mt-3 text-sm font-semibold text-slate-900 dark:text-white">No Challenges Found</h3>
      <p className="mt-1 text-xs text-slate-400">
      ...
      <button
        ...
        className="btn-primary mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-900 px-4 py-2 text-xs font-semibold text-white shadow-sm transition-all hover:bg-blue-800 dark:bg-blue-800 dark:text-white dark:hover:bg-blue-700"
      >
