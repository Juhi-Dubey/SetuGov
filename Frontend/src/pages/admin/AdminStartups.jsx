...
const STATUS_CONFIG = {
  ...
  SUBMITTED: { label: "Submitted", color: "bg-sky-50 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300" },
  ...
  REJECTED: { label: "Rejected", color: "bg-rose-50 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300" },
  CORRECTION_REQUESTED: { label: "Correction Requested", color: "bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300" },
};
  ...
  return (
    ...
    >
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <SummaryCard icon={Building2} title="Total Sellers" value={startups.length} type="primary" />
        ...
        <SummaryCard icon={AlertTriangle} title="Correction Needed" value={correctionCount} type="warning" />
        ...
        <div className="border-b border-slate-200 p-4 dark:border-slate-800 sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              aria-label="Refresh startup list"
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 transition-colors"
            >
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-200 text-left text-slate-900 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-100">
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Organization</th>
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Constitution</th>
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Statutory IDs</th>
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Email Status</th>
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Verification Status</th>
                <th className="px-6 py-2.5 text-left text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Registered</th>
                <th className="px-6 py-2.5 text-right text-xs font-bold uppercase tracking-wider text-slate-900 dark:text-slate-100">Action</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="max-w-[200px] truncate text-xs font-bold text-slate-900 dark:text-white">
                            {s.company_name}
                          </p>
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1 text-xs font-mono text-slate-600 dark:text-slate-400">
                        <div>PAN: <span className="font-semibold text-slate-900 dark:text-slate-100">{s.pan_number || "—"}</span></div>
                        {s.dpiit_number && (
                          <div>DPIIT: <span className="font-semibold text-indigo-600 dark:text-indigo-400">{s.dpiit_number}</span></div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                        isEmailVerified ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300" : "bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300"
                      }`}>
                        {isEmailVerified ? <CheckCircle2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-2xl border border-indigo-100 bg-indigo-50/40 p-4 dark:border-indigo-900/40 dark:bg-indigo-950/20">
                    ...
                    {dossier.correction_notes && (
                      <div className="mt-3 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
                        <span className="font-bold">Active Correction Notice:</span> {dossier.correction_notes}
                      </div>
                    )}

                    {dossier.rejection_reason && (
                      <div className="mt-3 rounded-xl bg-rose-50 p-3 text-xs text-rose-800 dark:bg-rose-950/40 dark:text-rose-300">
                  ...
                  {actionModal && (
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                      ...
                      <textarea
                        ...
                        className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs text-slate-900 outline-none focus:border-indigo-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white"
                      />
                      <div className="mt-2 flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => setActionModal(null)}
                          className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
                        >
                          Cancel
                        </button>
                        <button
...
function DossierItem({ label, value, mono }) {
  return (
    <div className="rounded-xl bg-slate-50 p-2.5 dark:bg-slate-900">
      ...
      <p className={`mt-0.5 truncate text-xs font-semibold text-slate-900 dark:text-slate-100 ${mono ? "font-mono font-bold" : ""}`}>
        {value || "—"}
      </p>
    </div>
  );
